...
export default function ConcentricView() {
  ...
  const transitionToByArticle = new Map(
    computeColumnPositionsForTransition(transitioningArticles, 0, size.width, size.height).map(
      (position) => [position.article.id, columnToMorph(position.x, position.y)] as const,
    ),
  );
